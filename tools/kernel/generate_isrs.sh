...
echo "//" >> $ISRS_GEN_C
echo "void ${ISRS_ARCH}_isr_gates_init()" >> $ISRS_GEN_C
echo "{" >> $ISRS_GEN_C
for index in $( seq 0 255 ); do
	echo "    ${ISRS_ARCH}_idt_set_gate( ${index}, ${ISRS_ARCH}_isr_stub_${index}, GDT_KERNEL_CODE_SEGMENT, IDT_FLAG_RING0 | IDT_FLAG_GATE_32_64BIT_INTERRUPT );" >> $ISRS_GEN_C
done
echo "}" >> $ISRS_GEN_C
